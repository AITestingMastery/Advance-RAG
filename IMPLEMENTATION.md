...
Same philosophy as `verify.py`: seed a scratch SQLite+Chroma store (never the real `storage/rag.db`) with a handful of fake employee records, monkeypatch `embed`/`classify_query`/`generate_answer`/`generate_aggregation_answer` with deterministic fakes, and prove `evaluate_rag.py`'s *scoring logic* — not the real LLM's accuracy — is correct against the real `process_document`/`apply_filters`/`query_document` code paths. Covers `structured_filter`, `aggregation`, `exact_lookup`, `live_lookup`'s `city_correct` scoring (both a correct-city and a deliberately-mismatched-city case, so a false pass can't sneak through), and `run_forced_hybrid_question()`'s monkeypatch-and-restore behavior (verifying both that forcing hybrid actually overrides the fake router, and that `classify_query` is genuinely restored afterward rather than leaking into later questions). This test needs no `OPENAI_API_KEY` and makes no network calls; it does not tell you whether the real LLM router is accurate, only that the harness measures it correctly once pointed at a live run.

### Scoring `live_lookup` (v1.1.1+)

`live_lookup` gets the same router-accuracy tracking every other strategy already had for free (`STRATEGY_LABEL_TO_KEY` maps `"Live data lookup"` → `"live_lookup"`, and `router_correct` is just `actual_strategy == expected_strategy`, unchanged). What it needed *in addition* is its own version of `count_correct`: `city_correct`, computed from `result["live_evidence"]["city"]` against an independently hand-specified `expected_city` in `GROUND_TRUTH` — same isolation principle as `oracle_filters`, never trusting the router's/extractor's own output as its own check. Two ground-truth rows (`LIVE-1`, `LIVE-2`) exercise this against the **real** OpenWeatherMap API when `evaluate_rag.py` runs live, so this genuinely requires `OPENWEATHER_API_KEY` to be set to score anything other than a graceful "not configured" miss — same requirement as `OPENAI_API_KEY` for every other question. RAGAS is not run on `live_lookup` rows, same rationale as the other three non-hybrid strategies: the evidence handed to `generate_answer()` is already a single verified API reading, not free-form retrieval, so there's nothing an LLM-judged faithfulness score would tell you that `city_correct` doesn't already cover more precisely.

## Live data (v1.1): the fifth strategy, `live_lookup`

Every strategy up to this point answers from data ingested at upload time — a closed snapshot. `live_lookup` is the first strategy that reaches outside that snapshot: it answers questions about real-world conditions that change faster than any re-ingestion cycle could keep up with (starting with current weather, via OpenWeatherMap), by calling a live API at question time instead of retrieving anything previously stored.

Two design rules kept deliberately strict, both learned from mistakes elsewhere in this codebase:

- **Never persist a live reading as if it were stable document content.** A temperature is stale within minutes; embedding it into Chroma or writing it into SQLite the way ingested records are cached would silently start lying within the hour. `live_data.py`'s only cache is a 60-second in-memory TTL purely to avoid re-hitting the API on back-to-back questions about the same city — never a durable store.
- **A failure in the live path must degrade to "insufficient evidence," never a crash or a guess.** `live_data.get_current_weather()` returns `{"error": "..."}` on every failure mode (missing API key, unknown city, network error, non-200 response) instead of raising, and `live_weather_answer()` hands that straight to `generate_answer()`, which already refuses to invent facts when evidence is empty — the same grounding discipline every other strategy relies on.

**A real structural bug this caused, and the fix.** `query_document()` used to `raise RuntimeError("No documents are indexed.")` as its very first line, before the router ever ran — meaning a weather question was unreachable on a fresh install with nothing uploaded yet, since the function never got far enough to classify the question as `live_lookup` at all. Fixed by classifying first (against an empty record set) when no documents exist, and only raising if the resolved strategy actually needs ingested records.

**City extraction is a known simplification, not the router's job (yet).** Rather than extend the Structured Outputs schema, `live_data.extract_city()` uses a regex over "in/for/at <city>" phrasing — the same pragmatic style as `infer_filters_from_question()`. This shipped with one real bug caught by its own offline test: "what is the weather in Hyderabad right now?" initially captured "Hyderabad right now" as the city, because the non-greedy match had no way to know a city name had ended without a punctuation mark or a recognized trailing filler word ("right now", "today", "currently", ...) to stop at. A production version should have the router extract the location as a structured field the same way it extracts filter values today, rather than a second, independent regex pass.

Offline coverage lives in `test_live_data_offline.py`, same philosophy as `verify.py` and `test_evaluate_rag_offline.py`: a fake `requests.get` (no real network call, no API key needed) proves the success path, the cache, and all three failure modes; a fake `classify_query` proves `query_document()`'s `live_lookup` dispatch end to end, including — the case that matters most — with zero documents indexed.
